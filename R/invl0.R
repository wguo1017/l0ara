#' fit a inverse gaussian regression model with l0 penalty
#' @description l0 iterative inverse gaussian regression
#' @usage invl0(X, y, lam, maxit, eps)
...
invl0 <- function(X, y, lam, maxit, eps){
  n <- nrow(X)
  m <- ncol(X)
